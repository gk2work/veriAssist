...
module fifo_checker (
    ...
    always @(posedge clk) begin
        if (!(!rst_n)) begin
            assert_not_full_and_empty: assert(!(full && empty));
        end
    end
    ...
    always @(posedge clk) begin
        if (!rst_n) begin
            ...
            if (_va_assert_write_inc_triggered_1 && !(count == count_past1 + 1))
                assert_write_inc: assert(0); // FAIL: consequent not met after antecedent
    ...
    always @(posedge clk) begin
        if (!rst_n) begin
            ...
            if (_va_assert_read_dec_triggered_2 && !(count == count_past1 - 1))
                assert_read_dec: assert(0); // FAIL: consequent not met after antecedent
    ...
    always @(posedge clk) begin
        if (!(!rst_n)) begin
            assert_count_pos: assert(count >= 0);
        end
    end
    ...
    always @(posedge clk) begin
        if (!rst_n) begin
            ...
            if (_va_assert_count_stable_triggered_3 && !(count == count_prev))
                assert_count_stable: assert(0); // FAIL: consequent not met after antecedent
