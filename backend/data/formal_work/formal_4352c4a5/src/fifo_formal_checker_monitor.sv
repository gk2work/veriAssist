...
module fifo_formal_checker (
    ...
    // Original SVA: !wr_en |-> ##1 rd_en[->1] ##0 (rd_data == $stable(wr_data))
    reg [0:0] _va_cover_fifo_underfill_delay_1;
    always @(posedge clk) begin
        if (!rst_n) begin
            _va_cover_fifo_underfill_delay_1 <= 0;
        end else begin
            _va_cover_fifo_underfill_delay_1 <= {_va_cover_fifo_underfill_delay_1[-1:0], (!wr_en)};
            if (_va_cover_fifo_underfill_delay_1[0] && (rd_en[->1] (rd_data == (wr_data == wr_data_prev))))
                cover_fifo_underfill: cover(1);
        end
    end

    // WARNING: property 'p_fifo_no_overflow' not found for assert_fifo_no_overflow
    // WARNING: property 'p_fifo_full_empty' not found for assert_fifo_full_empty
endmodule
`endif // FORMAL