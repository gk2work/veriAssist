...
module fifo_sva (
    ...
    always @(posedge clk_in) begin
        if (!rst_n_in) begin
            ...
            rst_n_in_past1 <= rst_n_in;
            rd_en_in_past1 <= rd_en_in;
            empty_out_past1 <= empty_out;
            full_out_past1 <= full_out;
    ...
    //   ($past(rd_en_in) &&
    //    !$past(empty_out) &&
    //    !($past(wr_en_in) && (!$past(full_out) || $past(rd_en_in))))
    always @(posedge clk_in) begin
        if (!(!rst_n_in)) begin
            if ((empty_out && !empty_out_prev) &&
   rst_n_in_past1 &&
   rst_n_in &&
   !(rst_n_in && !rst_n_in_prev) && !(rd_en_in_past1 &&
   !empty_out_past1 &&
   !(wr_en_in_past1 && (!full_out_past1 || rd_en_in_past1))))
    ...
    //         (wr_en_in) ##1
    //         (rd_en_in)) |->
    //       ( ##1 (rd_data_out == $past(wr_data_in, 1))) )
    ...
    always @(posedge clk_in) begin
        if (!rst_n_in) begin
            ...
            if (_va_ap_fifo_data_after_empty_trig_7 && !(rd_data_out == wr_data_in_past1))
