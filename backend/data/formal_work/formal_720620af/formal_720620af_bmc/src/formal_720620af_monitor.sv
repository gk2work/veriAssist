...
module fifo (
    ...
    // SKIPPED: property uses constructs not yet fully lowered (residual SVA: ['$fell('])
    ...
    // SKIPPED: property uses constructs not yet fully lowered (residual SVA: ['$onehot0('])
    ...
    // SKIPPED: property uses constructs not yet fully lowered (residual SVA: ['$rose(', '$past('])
    ...
    // SKIPPED: property uses constructs not yet fully lowered (residual SVA: ['$rose(', '$past(', '$past('])
    ...
    // SKIPPED: property uses constructs not yet fully lowered (residual SVA: ['$rose('])
    // Original SVA: ( ($rose(rst_n_in))|->(empty_out) )

    // --- _auto_assert_fifo_read_write_equal: assert property (fifo_read_write_equal) ---
    // SKIPPED: property uses constructs not yet fully lowered (residual SVA: ['$stable(', '$stable('])
    ...
    // SKIPPED: property uses constructs not yet fully lowered (residual SVA: ['[*32]', '[*32]'])
    // Original SVA: ( ((wr_en_in && !rd_en_in) [*32])|->( ##1 full_out) )

    // --- _auto_assert_fifo_data_after_empty: assert property (fifo_data_after_empty) ---
    // SKIPPED: property uses constructs not yet fully lowered (residual SVA: ['$past('])
    // Original SVA: ( ((empty_out) &&(wr_en_in) ##1 (rd_en_in))|-> ( ##1 (rd_data_out == $past(wr_da
