...
module fifo (
    ...
    always @(posedge clk) begin
        if (!(!rst_n_in)) begin
            if (( (empty_out)) test1: assume((!rd_en_in) ));
        end
    end
    ...
    always @(posedge clk) begin
        if (!(!rst_n_in)) begin
            if ((1 && ((full_out) ))) _auto_cover_fifo_full: cover(1);
        end
    end
    ...
    always @(posedge clk) begin
        if (!(!rst_n_in)) begin
            if ((1 && ((empty_out) ))) _auto_cover_fifo_empty: cover(1);
        end
    end
    ...
    always @(posedge clk) begin
        if (!(!rst_n_in)) begin
            if ((1 && (((!full_out && full_out_prev)) ))) _auto_cover_fifo_full_falling_edge: cover(1);
        end
    end
    ...
    always @(posedge clk) begin
        if (!(!rst_n_in)) begin
            if (( (full_out)) _auto_assume_fifo_full_no_write: assume((!wr_en_in) ));
        end
    end
