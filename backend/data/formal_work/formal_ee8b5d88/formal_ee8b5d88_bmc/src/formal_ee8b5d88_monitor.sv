...
module fifo (
    ...
    always @(posedge clk_in) begin
        if (!(!rst_n_in)) begin
            if (1 && !(({full_out,empty_out}) == 0 || (({full_out,empty_out}) & (({full_out,empty_out}) - 1)) == 0))
                ap_fifo_not_full_empty: assert(0);
        end
    end
  ...
  |->
    ...
    always @(posedge clk_in) begin
        if (!(!rst_n_in)) begin
            if (1 && !(({full_out, empty_out}) == 0 || (({full_out, empty_out}) & (({full_out, empty_out}) - 1)) == 0))
                ap_fifo_onehot0_full_empty: assert(0);
        end
    end
    ...
    always @(posedge clk_in) begin
        if (!(!rst_n_in)) begin
            if (full_out && !full_out_prev && !(wr_en_in_past1))
                ap_fifo_full_prev_write: assert(0);
        end
    end
  ...
  |->
    ( ##1 !empty_out) )
    reg [0:0] _va_ap_fifo_write_not_empty_delay_2;
    always @(posedge clk_in) begin
        if (!rst_n_in) begin
            _va_ap_fifo_write_not_empty_delay_2 <= 0;
        end else begin
            _va_ap_fifo_write_not_empty_delay_2 <= {_va_ap_fifo_write_not_empty_delay_2[-1:0], (wr_en_in)};
            if (_va_ap_fifo_write_not_empty_delay_2[0] && !(!empty_out))
                ap_fifo_write_not_empty: assert(0); // FAIL at delay 1
        end
    end
    ...
    always @(posedge clk_in) begin
        if (!(!rst_n_in)) begin
            if (rst_n_in && !rst_n_in_prev && !(empty_out))
                ap_fifo_empty_after_reset: assert(0);
        end
    end
  ...
  |->
    ( ##1 $stable(full_out) &&
    ($stable(empty_out))) )
    reg [0:0] _va_ap_fifo_read_write_equal_delay_3;
    always @(posedge clk_in) begin
        if (!rst_n_in) begin
            _va_ap_fifo_read_write_equal_delay_3 <= 0;
        end else begin
            _va_ap_fifo_read_write_equal_delay_3 <= {_va_ap_fifo_read_write_equal_delay_3[-1:0], (rd_en_in == wr_en_in)};
            if (_va_ap_fifo_read_write_equal_delay_3[0] && !((full_out == full_out_prev) &&
  ...
  |->
    ...
    reg [0:0] _va_ap_fifo_data_after_empty_delay_5;
    always @(posedge clk_in) begin
        if (!rst_n_in) begin
            _va_ap_fifo_data_after_empty_delay_5 <= 0;
        end else begin
            _va_ap_fifo_data_after_empty_delay_5 <= {_va_ap_fifo_data_after_empty_delay_5[-1:0], ((empty_out) &&
    (wr_en_in) ##1
    (rd_en_in))};
            if (_va_ap_fifo_data_after_empty_delay_5[0] && !(rd_data_out == wr_data_in_past2))
                ap_fifo_data_after_empty: assert(0); // FAIL at delay 1
        end
    end

    // --- ap_fifo_data_after_empty_1_4: assert property (fifo_data_after_empty_1_4) ---
    // SKIPPED: property uses constructs not yet fully lowered (local variables in sequence)
