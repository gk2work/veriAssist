...
module fifo_sva (
    ...
    always @(posedge clk_in) begin
        if (!(!rst_n_in)) begin
            _auto_assert_fifo_not_full_empty: assert(({full_out, empty_out}) == 0 || (({full_out, empty_out}) & (({full_out, empty_out}) - 1)) == 0);
        end
    end
    ...
    always @(posedge clk_in) begin
        if (!(!rst_n_in)) begin
            if (empty_out && !(!rd_en_in))
                begin end // disabled assertion _auto_assert_fifo_empty_no_read
        end
    end
    ...
    // WARNING: property 'fifo_empty_prev_read_no_write' not found for _auto_assert_fifo_empty_prev_read_no_write
